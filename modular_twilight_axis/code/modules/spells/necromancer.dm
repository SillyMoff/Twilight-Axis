...
/obj/effect/proc_holder/spell/invoked/raise_to_skeleton/cast(list/targets, mob/living/carbon/human/user)
	...
	if(istype(obj, /mob/living/carbon/human/species/goblin))
		...
		to_chat(user, span_warning("I cannot raise this body, need else"))
		return FALSE
	else if(istype(obj, /mob/living/carbon/human/species/human/northern/bog_deserters))
		to_chat(user, span_warning("I cannot raise this body, need else"))
		return FALSE
	...
	if(user.mind.boneboys < user.mind.bonemax)
		...
		to_chat(user, span_warning("I cannot sustain another self aware skeleton. Previous sketelons should be perished."))
		return TRUE
...
/mob/living/carbon/human/proc/turn_to_minion(mob/living/carbon/human/master, ckey)
	...
	update_body()
	src.mind.AddSpell(new /obj/effect/proc_holder/spell/self/suicidebomb)
